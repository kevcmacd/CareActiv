...
CREATE TABLE IF NOT EXISTS `care_activity_mgr`.`clients` (
  ...
  CONSTRAINT `fk_clients_appointments1`
    FOREIGN KEY (`id`)
    REFERENCES `care_activity_mgr`.`appointments` (`patient_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_clients_routines1`
    FOREIGN KEY (`id`)
    REFERENCES `care_activity_mgr`.`routines` (`patient_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_clients_medication1`
    FOREIGN KEY (`id`)
    REFERENCES `care_activity_mgr`.`medication` (`patient_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;
...
CREATE TABLE IF NOT EXISTS `care_activity_mgr`.`users` (
  ...
  CONSTRAINT `fk_users_clients`
    FOREIGN KEY (`id`)
    REFERENCES `care_activity_mgr`.`clients` (`userid`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
AUTO_INCREMENT = 3
